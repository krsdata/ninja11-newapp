...
import mega.cricks.ui.contest.MoreContestFragment
...
import mega.cricks.databinding.ActivityMoreContestBinding
...
class MoreContestActivity : BaseActivity(), OnContestLoadedListener, OnContestEvents,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        matchObject = intent.getSerializableExtra(ContestActivity.SERIALIZABLE_KEY_UPCOMING_MATCHES) as UpcomingMatchesModel
        allContestList = intent.getSerializableExtra(ContestActivity.SERIALIZABLE_KEY_JOINED_CONTEST) as ArrayList<ContestsParentModels>
        selectedObject = intent.getSerializableExtra(SERIALIZABLE_KEY_LIST_POSTIION) as ContestsParentModels
...
//        })
    ...
    fun getAllContest() {
        ...
        WebServiceClient(this@MoreContestActivity).client.create(IApiMethod::class.java).getContestByMatch(models)
...
//                            mListener.onMyContest(responseModel.joinedContestDetails!!)
                            allContestList = responseModel.matchContestlist as ArrayList<ContestsParentModels>?
                            setupViewPager(mBinding!!.viewpagerContest)
                        }else {
                            MyUtils.showToast(this@MoreContestActivity,"No Contest available for this match "+res.toString())
                        }
                    }
                }
    ...
    private fun setupViewPager(viewPager: ViewPager) {
        ...
        for(i in 0..allContestList!!.size-1) {
            ...
            adapter.addFragment(
                MoreContestFragment.newInstance(bundle), clobject.contestTitle+"("+clobject!!.allContestsRunning!!.size+")"
            )
                ...
                override fun onResponse(
                    ...
                    if(res!=null) {
                        if(res.actionForTeam==1){
                            val intent = Intent(this@MoreContestActivity, CreateTeamActivity::class.java)
                            ...
                            val intent = Intent(this@MoreContestActivity, SelectTeamActivity::class.java)
                            intent.putExtra(CreateTeamActivity.SERIALIZABLE_MATCH_KEY, matchObject)
                            ...
                            Toast.makeText(this@MoreContestActivity,res.message,Toast.LENGTH_LONG).show()
                        }
                    }
